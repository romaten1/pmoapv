...
use app\modules\admin\models\RozkladForm;
 ...
 */
class RozkladController extends Controller
{
        ...
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'allow'   => true,
                        'actions' => [ 'index', 'create', 'update', 'view', 'delete', 'excel' ],
                        'roles'   => [ 'admin' ],
                    ]
                ],
            ],
    ...
    public function actionExcel(  )
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($model->file && $model->validate()) {
                $file_name      = 'rozklad_faculty';
                $file_full_name = $file_name . '.' . $model->file->extension;
                $model->file->saveAs('uploads/rozklad/' . $file_full_name);
                $result = 'Загружено';
            }
            if ($model->file2 && $model->validate()) {
                $file_name      = 'rozklad_teachers';
                $file_full_name = $file_name . '.' . $model->file2->extension;
                $model->file2->saveAs('uploads/rozklad/' . $file_full_name);
                $result = 'Загружено';
            }
